...
def main():
    #TODO: Verify why the model is infeasible
    ...
    w1 = Router(node_name="w1", node_label="Router")
    w2 = Router(node_name="w2", node_label="Router", is_gateway=True)
    ...
    net.add_network_node(w2, name="w2", label="Router")
    ...
    net.add_network_edge(PhysicalLink(source=w1, target=s2, link_specs=LinkSpecs(bandwidth=100,
                                                                                 propagation_delay=1E-6)))
    net.add_network_edge(PhysicalLink(source=w2, target=w1, link_specs=LinkSpecs(bandwidth=200,
                                                                                 propagation_delay=1E-6)))
    # Create request 1
    vm0 = DummyVirtualMachine(gateway_router=w2, node_name=f"vm0_{VMRequest.get_new_request_id()}",
    ...
    v_net1_edges[0].add_hosting_physical_link(net.network_edges[2])
